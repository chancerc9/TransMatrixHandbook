...
```python
class TickMatcherDetail(BaseMatcher):
    ...
    def match_submitting(self, order: Order) -> MatchResult:
        ...
        # 委托指令到达市场时触发
        # 策略发单 ---> 延迟（参数可设置）---> 市场收到委托 ---> 触发 match_submitting ---> 延迟 ---> 策略收到回报
        
        data = self.data[order.code]
        next_tick = data[data.curloc + 1]                   # 下一个 tick 的数据
        trade_value, trade_volume = 0, 0                    # 用于计算成交均价
        
        if order.direction == ORDER_DIRECTION.BUY: 
            
            if order.price > self.limit_up_prices.get(order.code, None):
                order.status = ORDER_STATUS.REJECTED
                return MatchResult(order = order, event = MATCH_EVENT.REJECT)
            
            # 撮合对手盘口
            for price, volume in zip(
                next_tick[SLOT_AP], next_tick[SLOT_AV]
            ):
                
                if order.price >= price:
                    volume = min(order.pending_volume, volume)
                    order.pending_volume -= volume
                    trade_value += price * volume
                    trade_volume += volume
                    if order.pending_volume == 0:
                        order.status = ORDER_STATUS.ALL_TRADE
                        order.front_volume = 0
                        return MatchResult(
                            order = order, 
                            event = MATCH_EVENT.ALL_TRADE, 
                            price = trade_value / trade_volume, 
                            volume = trade_volume
                        )
                        
                else: break
                        
            if order.pending_volume < order.volume:
                order.status = ORDER_STATUS.PARTIAL_TRADE
                order.front_volume = 0
                return MatchResult(
                    order = order, 
                    event = MATCH_EVENT.PARTIAL_TRADE, 
                    price = trade_value / trade_volume, 
                    volume = trade_volume
                )
            ...
            # 本方盘口 Pending
            for price, volume in zip(
                next_tick[SLOT_BP], next_tick[SLOT_BV]
            ):
                if order.price == price: 
                    order.status = ORDER_STATUS.PENDING
                    order.front_volume = volume
            ...
            print(f'{self.__class__}: {order.id} is rejected (撮合机制不允许 {MAX_LEVEL} 深度以外的本方价格)。委托价格: {order.price}, 买方第 {MAX_LEVEL} 档价格: {price}。')
            return MatchResult(order = order, event = MATCH_EVENT.REJECT)
                
        else: # SELL
            
            if order.price < self.limit_down_prices.get(order.code, None):
                order.status = ORDER_STATUS.REJECTED
                return MatchResult(order = order, event = MATCH_EVENT.REJECT)
            ...
            print(f'{self.__class__}: {order.id} is rejected (撮合机制不允许 {MAX_LEVEL} 深度以外的本方价格)。委托价格: {order.price}, 卖方第 {MAX_LEVEL} 档价格: {price}。')
            return MatchResult(order = order, event = MATCH_EVENT.REJECT)
    ...
    def match_pending(self, order: Order) -> MatchResult:
        ...
        # tick 数据更新时触发
        # 市场 tick 数据更新 ---> 触发 match_pending ---> 延迟 ---> 策略收到回报
        
        # loc 指向下一个 tick
        data = self.data[order.code]
        tick = data.get()
        next_tick = data[data.curloc + 1]
        bid_price_1, ask_price_1 = tick[SLOT_P_TOP]
        
        # 对于买单，撮合市场主卖量
        if order.direction == ORDER_DIRECTION.BUY:
            
            volume0, turnover0 = tick[SLOT_VT]
            volume1, turnover1 = next_tick[SLOT_VT]
            _, act_volume = est_act_vol(volume0, volume1, turnover0, turnover1, bid_price_1, ask_price_1)
            price_volume = zip(tick[SLOT_BP], tick[SLOT_BV])
            
            for price, volume in price_volume:
                
                if order.price < price: 
                    act_volume -= volume
                    if act_volume <= 0: return MatchResult(order = order, event = MATCH_EVENT.NOTHING)
                    
                else:
                    if order.front_volume <= act_volume:
                        delta = act_volume - order.front_volume
                        order.front_volume = 0
                        if delta >= order.pending_volume:
                            volume = order.pending_volume
                            order.pending_volume = 0
                            order.front_volume = 0
                            order.status = ORDER_STATUS.ALL_TRADE
                            return MatchResult(order = order, event = MATCH_EVENT.ALL_TRADE, price = order.price, volume = volume)
                        else:
                            order.status = ORDER_STATUS.PARTIAL_TRADE
                            order.pending_volume = order.pending_volume - delta
                            order.front_volume = 0
                            return MatchResult(order = order, event = MATCH_EVENT.PARTIAL_TRADE, price = order.price, volume = delta)
                    else: 
                        order.front_volume -= act_volume    
        ...
        # 对于卖单，撮合市场主买量
            ...
            volume0, turnover0 = tick[SLOT_VT]
            volume1, turnover1 = next_tick[SLOT_VT]
            act_volume, _ = est_act_vol(volume0, volume1, turnover0, turnover1, bid_price_1, ask_price_1)
            price_volume = zip(tick[SLOT_AP], tick[SLOT_AV])
    ...
    def match_cancelling(self, order: Order) -> MatchResult:
        ...
        # 撤单指令到达市场时触发
        # 策略撤单 ---> 延迟（参数可设置）---> 市场收到委托 ---> 触发 match_cancelling ---> 延迟 ---> 策略收到回报
        data = self.data[order.code]
        tick = data.get()
        next_tick = data[data.curloc + 1]
        bid_price_1, ask_price_1 = tick[SLOT_P_TOP]
                ...
                if order.price < price: 
                    ...
                    if order.front_volume <= act_volume:
                        ...
                        if delta >= order.pending_volume:
                            ...
                            order.status = ORDER_STATUS.CANCELED
                            order.pending_volume -= delta
                            return MatchResult(order = order, event = MATCH_EVENT.PARTIAL_CANCEL, price = order.price, volume = delta)
                    else: 
                        order.front_volume -= act_volume    
                        order.status = ORDER_STATUS.CANCELED
                        return MatchResult(order = order, event = MATCH_EVENT.CANCEL)
    ...
    def match_partial_traded(self, order: Order) -> MatchResult:
        ...
        if act_volume > 0:   
            if act_volume >= order.pending_volume:
                order.status = ORDER_STATUS.ALL_TRADE; order.pending_volume = 0
                return MatchResult(order = order, event = MATCH_EVENT.ALL_TRADE, price = order.price, volume = order.pending_volume)
            else:
                order.pending_volume -= act_volume
                return MatchResult(order = order, event = MATCH_EVENT.PARTIAL_TRADE, price = order.price, volume = act_volume)
        return MatchResult(order = order, event = MATCH_EVENT.NOTHING)
```
...
class OrderFlowMatcher(BaseMatcher):
    
    data_type = 'orderflow'
    
    def match_submitting(self, order:Order) -> MatchResult:
        
        # print(self.market._matrix.time, 'match submitting',
        # order.id, order.code, order.direction, order.price, order.volume)
         
        code = order.code
        tick = self.data[code].get()
        order.seq_num = self.ist_seqnum[code] + 0.5 
        trade_value, trade_volume = 0, 0
        
        if order.direction == ORDER_DIRECTION.BUY:
            
            if order.price > self.limit_up_prices.get(code, None):
                order.status = ORDER_STATUS.REJECTED
                return MatchResult(order = order, event = MATCH_EVENT.REJECT)
            
            for price, volume in zip(
                tick[SLOT_AP], tick[SLOT_AV]
            ):
                if order.price >= price:
                    volume = min(order.pending_volume, volume)
                    order.pending_volume -= volume
            ...
            if order.price < self.limit_down_prices.get(code, None):
                order.status = ORDER_STATUS.REJECTED
                return MatchResult(order = order, event = MATCH_EVENT.REJECT)
    ...
    def match_pending(self, order:Order) -> MatchResult:
        ...
        data = self.data[code]
        tick = data.get()
        bid_seq_num, ask_seq_num, event, price, volume, side = tick[SLOT_PENDING_INFO]
        
        if event == ORDER_DATA_DYPE: return MatchResult(order = order, event = MATCH_EVENT.NOTHING)
        
        if event == CANCEL_DATA_DYPE: 
            if price == order.price:
                if side == 1 \
                and order.direction == ORDER_DIRECTION.BUY \
                and bid_seq_num < order.seq_num:
                    order.front_volume -= volume
                    assert order.front_volume >= 0
                    if order.front_volume == 0:
                        order.status = ORDER_STATUS.PARTIAL_TRADE
                    return MatchResult(order = order, event = MATCH_EVENT.NOTHING)
                
                if side == 2 \
                and order.direction == ORDER_DIRECTION.SELL \
                and ask_seq_num < order.seq_num:
                    order.front_volume -= volume
                    assert order.front_volume >= 0
                    if order.front_volume == 0:
                        order.status = ORDER_STATUS.PARTIAL_TRADE
        ...
        # TRADE
        if order.direction == ORDER_DIRECTION.BUY and side == 2:
            
            if volume == tick[ACTION_TRADE_VOLUME]:
            
                if order.price >= price:
                    if order.front_volume <= volume:
                        delta = volume - order.front_volume
                        order.front_volume = 0
                        if delta >= order.pending_volume:
                            volume = order.pending_volume
                            order.pending_volume = 0
                            ...
                            order.status = ORDER_STATUS.ALL_TRADE
                            return MatchResult(
                                order = order, event = MATCH_EVENT.ALL_TRADE, 
                            ...
                            order.front_volume = 0
                            return MatchResult(
                                order = order, event = MATCH_EVENT.PARTIAL_TRADE,
                                price = order.price, volume = delta
                            )
                    else:
                        order.front_volume -= volume
                        return MatchResult(order = order, event = MATCH_EVENT.NOTHING)
                ...
                last_tick = data[data.curloc - 1]
                for _price, _volume in zip(
                    last_tick[SLOT_BP], last_tick[SLOT_BV]
                ):
                    if order.price < _price:
                        volume -= _volume
                        if volume <= 0: return MatchResult(order = order, event = MATCH_EVENT.NOTHING)
        ...
        elif order.direction == ORDER_DIRECTION.SELL and side == 1:
            ...
            if volume == tick[ACTION_TRADE_VOLUME]:
                ...
                last_tick = data[data.curloc -1]
                for _price, _volume in zip(
                    last_tick[SLOT_AP], last_tick[SLOT_AV]
                ):
                    if order.price > _price:
                        volume -= _volume
                        if volume <= 0: return MatchResult(order = order, event = MATCH_EVENT.NOTHING)
    ...
    def match_partial_traded(self, order: Order) -> MatchResult:
        ...
        # print(self.market._matrix.time, 'match_partial_traded', 
        # order.id, order.code, order.direction, order.price, order.volume)
        
        data = self.data[order.code]
        tick = data.get()
        event, price, volume, side = tick[SLOT_TRADE_INFO]
        
        if event == TRADE_DATA_DYPE: 
            if order.direction == ORDER_DIRECTION.BUY and side == 2: 
                if volume == tick[ACTION_TRADE_VOLUME]:
                    
                    if order.price >= price:
                        if volume >= order.pending_volume:
                            volume = order.pending_volume
                            order.pending_volume = 0
                            order.front_volume = 0
                            order.status = ORDER_STATUS.ALL_TRADE
                            ...
                            order.status = ORDER_STATUS.PARTIAL_TRADE
                            order.pending_volume = order.pending_volume - volume
                            order.front_volume = 0
                            return MatchResult(
                                order = order, event = MATCH_EVENT.PARTIAL_TRADE,
                                price = order.price, volume = volume
            ...
            if order.direction == ORDER_DIRECTION.SELL and side == 1:
                if volume == tick[ACTION_TRADE_VOLUME]:
                ...
                if order.price == price:
                    if order.front_volume <= volume:
                        ...
                        if delta >= order.pending_volume:
                            ...
                            order.front_volume = 0
                            return MatchResult(
                                order = order, event = MATCH_EVENT.PARTIAL_CANCEL,
                                price = order.price, volume = delta
                            )
                    else:
                        order.status = ORDER_STATUS.CANCELED
